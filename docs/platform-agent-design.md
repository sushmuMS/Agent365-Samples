...
| 9 | Recurring task scheduling | **Graph Calendar Series (Option 2)** — `create_trigger_definition(eventType="schedule")` creates a recurring Graph calendar event; smart-notifications fires `evaluate_event_triggers` on each occurrence. See [§10a](#10a-scheduling-design--graph-calendar-series) for full design. | Option 1: Azure Functions timer trigger — simpler to deploy but requires separate infrastructure |
...
## 10a. Scheduling Design — Graph Calendar Series

### Problem Statement

`mcp_TaskPersonalizationServer` is stateless — it evaluates triggers when called but cannot fire on a timer internally. The hero scenario requires "every Friday at 5:00 AM for 4 weeks" to be executed reliably without PlatformAgent polling or a separate scheduler process.

### Chosen Approach: Graph Calendar Series (Option 2)

When a calling agent creates a `schedule`-type trigger, `mcp_TaskPersonalizationServer` additionally calls the Graph API to create a **recurring calendar event series** on the agent's calendar. The existing `smart-notifications` Graph change notification pipeline — already used for email and Teams — fires `evaluate_event_triggers` on each calendar occurrence. No new infrastructure is required.

### End-to-End Flow

```
Design Time (First Call):
  PlatformAgent → create_trigger_definition(eventType="schedule",
                    ruleDefinition={frequency:"weekly", maxOccurrences:4},
                    mcpPromptInstructions="run the weekly campaign analysis")
      ├─ Saves AgentRule (EventType="schedule") to Dataverse
      └─ Calls Graph API: POST /users/{agentOid}/events
           recurrence: { pattern: {type:"weekly", daysOfWeek:["Friday"]},
                         range: {type:"numbered", numberOfOccurrences:4} }
           startDateTime: "2026-03-14T05:00:00"

Runtime (Every Friday 5:00 AM):
  Graph emits change notification → smart-notifications NotificationHandler
      └─ Agent SDK → ATG → mcp_TaskPersonalizationServer
           evaluate_event_triggers(eventType="schedule", {
             weekNumber: 2,
             frequency: "weekly",
             scheduledTime: "2026-03-21T05:00:00Z",
             isLastExecution: false
           })
      ├─ Queries Dataverse: AgentRule WHERE EventType="schedule" AND Enabled=true
      ├─ Matches rule → retrieves linked MCPPrompt instructions
      └─ Returns: { instructions: ["run the weekly campaign analysis"] }
  PlatformAgent loads task memory → executes → logs sentiment → sends email
```

### Why This Pattern Over Alternatives

| | Graph Calendar Series (chosen) | Azure Functions Timer |
|---|---|---|
| New infrastructure | None — reuses Graph notification pipeline | Requires Azure Functions deployment |
| Schedule storage | Graph calendar series (self-describing, user-visible) | `cronExpression` in `RuleDefinition` JSON + Azure portal config |
| Firing mechanism | Graph change notification (same path as email/Teams) | Timer trigger calling ATG directly |
| Cancellation | Delete calendar series via Graph | Delete Azure Function schedule + AgentRule |
| Visibility | Agent's calendar shows the recurring event — auditable | Hidden from users |
| Fits existing pattern | Fully — `smart-notifications` unchanged | Partially — requires new endpoint wiring |

### Changes Required in `mcp_TaskPersonalizationServer`

#### 1. Schedule event type in `EventSchemaService`

Add `"schedule"` to `GetEventTypes()` and add its schema:

```csharp
"schedule" => new EventTypeSchema
{
    EventType = "schedule",
    Description = "A recurring scheduled execution trigger fired by a Graph calendar event",
    Properties = new()
    {
        ["frequency"]       = new() { Type = "string",  Description = "e.g. 'weekly', 'daily'" },
        ["weekNumber"]      = new() { Type = "integer", Description = "Current iteration (1-based)" },
        ["maxWeeks"]        = new() { Type = "integer", Description = "Total occurrences configured" },
        ["scheduledTime"]   = new() { Type = "string",  Description = "ISO 8601 execution timestamp" },
        ["isLastExecution"] = new() { Type = "boolean", Description = "True on the final occurrence" }
    }
}
```

#### 2. `SchedulePropertyAccessor` (new file)

Implements `IPropertyAccessor` — extracts `frequency`, `weekNumber`, `maxWeeks`, `scheduledTime`, `isLastExecution` from the schedule event JSON. Registered in DI alongside `EmailPropertyAccessor`, `MessagePropertyAccessor`, `DocumentPropertyAccessor`.

#### 3. `create_trigger_definition` — Graph calendar creation (side effect)

When `eventType == "schedule"`, after saving the `AgentRule` to Dataverse, call:

```
POST https://graph.microsoft.com/v1.0/users/{agentOid}/events
{
  "subject": "Scheduled Agent Trigger: {ruleName}",
  "recurrence": {
    "pattern": { "type": "weekly", "daysOfWeek": ["Friday"] },
    "range":   { "type": "numbered", "numberOfOccurrences": 4, "startDate": "2026-03-14" }
  },
  "start": { "dateTime": "2026-03-14T05:00:00", "timeZone": "UTC" },
  "end":   { "dateTime": "2026-03-14T05:30:00", "timeZone": "UTC" },
  "isReminderOn": false
}
```

The `agentOid` comes from the HTTP context (same field used across all trigger tools). The Graph call uses the agent's own credentials — `Calendars.ReadWrite` permission required.

#### 4. `delete_trigger_definition` — Graph calendar cleanup

When a `schedule`-type rule is deleted, the associated Graph calendar series must also be deleted. The `AgentRule.RuleDefinition` JSON should store `graphEventId` (returned by the Graph POST above) so it can be cleaned up:

```json
{
  "conditions": [...],
  "graphEventId": "AAMkAGI2..."
}
```

### `smart-notifications` Changes (Minimal)

The `NotificationHandler` already handles Graph change notifications for mail and Teams. The only addition is subscribing to the agent's calendar:

```csharp
// In subscription setup — add calendar alongside existing subscriptions
await graphClient.Subscriptions.PostAsync(new Subscription
{
    ChangeType = "created,updated",
    NotificationUrl = webhookUrl,
    Resource = $"/users/{agentOid}/events",
    ExpirationDateTime = DateTimeOffset.UtcNow.AddDays(3),
    ClientState = clientState
});
```

When the notification fires, the handler calls `evaluate_event_triggers` with `eventType="schedule"` and extracts `weekNumber`, `scheduledTime`, and `isLastExecution` from the Graph event's `recurrence.range` data.

### Required App Registration Permissions

| Permission | Type | Purpose |
|---|---|---|
| `Calendars.ReadWrite` | Application | Create/delete recurring calendar event series |
| `Calendars.Read` | Application | Read calendar events in notification handler |

These are additive — they do not affect the existing `Mail.Read`, `Mail.Send`, `ChannelMessage.Read.All` permissions already required.

### Sequence Diagram

```
PlatformAgent                ATG / mcp_TaskPersonalizationServer    Graph API    smart-notifications
     │                                       │                          │                │
     ...
     │                                       │  {graphEventId: "AAMk.."} │               │
     │                                       │◄─────────────────────────│                │
     │                                       │  Store graphEventId in   │                │
     │                                       │  RuleDefinition JSON     │                │
     │  {success, ruleId, graphEventId}       │                          │                │
     │◄──────────────────────────────────────│                          │                │
     │                                       │                          │                │
     │                               [Every Friday 5:00 AM]             │                │
     │                                       │       Calendar event     │                │
     │                                       │       notification ──────┼───────────────►│
     │                                       │                          │                │
     ...
     │                                       │◄───────────────────────────────────────── │
     │                                       │  Match AgentRule         │                │
     │                                       │  Return MCPPrompt instructions             │
     │                                       │──────────────────────────────────────────►│
     │  load task memory + execute           │                          │                │
     │◄──────────────────────────────────────│                          │                │
```
...
|---|---|---|
| S1 | Graph subscription expiry | Calendar subscriptions expire every 3 days — `smart-notifications` must auto-renew |
| S2 | Agent calendar visibility | Recurring event appears on agent's calendar — decide if this is acceptable or if a service account calendar is preferred |
| S3 | `cronExpression` parsing | User says "every Friday at 5AM" — PlatformAgent LLM must convert this to a Graph recurrence pattern before calling `create_trigger_definition` |
| S4 | `Calendars.ReadWrite` permission | Must be added to the PlatformAgent app registration before schedule triggers can be created |
